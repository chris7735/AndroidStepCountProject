...
public class PedometerService extends Service implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        if (running){
            ...
            String strRestartDate = sharedPreferences.getString(Bridge.RESTART_DATE,"");
            boolean isStopCounts = sharedPreferences.getBoolean(Bridge.IS_STOP_COUNTS,false);
            Log.i(TAG, "onSensorChanged: currentDate: "+currentDate.toString());
            Log.i(TAG, "onSensorChanged: strRestartDate: "+strRestartDate);
            Log.i(TAG, "onSensorChanged: isStopCounts: "+isStopCounts);

            if (strRestartDate != null && !strRestartDate.isEmpty()) {
                SimpleDateFormat sdf = new SimpleDateFormat("EEE MMM dd HH:mm:ss z yyyy", java.util.Locale.ENGLISH);
                try {
                    Date restartDate = sdf.parse(strRestartDate);
                    if (restartDate == null) return;
                    Log.i(TAG, "onSensorChanged: restartDate: "+restartDate.toString());

                    if (currentDate.before(restartDate)) {
                        Bridge.initialSteps=(int) sensorEvent.values[0];
                        Log.i(TAG, "onSensorChanged: Counting was stopped" + Bridge.initialSteps);
                        return;
                    }
    ...
    public void saveData(int currentSteps) {
        ...
        editor.putBoolean(Bridge.IS_STOP_COUNTS,false);
        editor.apply();
    }
